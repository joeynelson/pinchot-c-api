...
#define JS50WSC_CAMERA_BRIGHTNESS_BIT_DEPTH (8)
#define JS50WSC_NUM_CAMERAS (JS_CAMERA_A + 1)
#define JS50WSC_NUM_LASERS (JS_LASER_0 + 1)
#define JS50WSC_NUM_ENCODERS (JS_ENCODER_2 + 1)
#define JS50WSC_MAX_CAMERA_HEIGHT (1088)
#define JS50WSC_MAX_CAMERA_WIDTH (1456)
                                      ...
                                      jsScanHeadCapabilities *c)
{
  ...
  switch (t) {
    ...
    case (JS_SCAN_HEAD_JS50WSC):
      // Remove this in the future
      static_assert(
        (JS50WSC_MAX_CAMERA_HEIGHT == JS_CAMERA_IMAGE_DATA_MAX_HEIGHT) &&
          (JS50WSC_MAX_CAMERA_WIDTH == JS_CAMERA_IMAGE_DATA_MAX_WIDTH),
        "camera max dimensions define mismatch");

      c->camera_brightness_bit_depth = JS50WSC_CAMERA_BRIGHTNESS_BIT_DEPTH;
      c->max_camera_image_height = JS50WSC_MAX_CAMERA_HEIGHT;
      c->max_camera_image_width = JS50WSC_MAX_CAMERA_WIDTH;
      c->max_scan_rate = kPinchotConstantMaxScanRate;
      c->num_cameras = JS50WSC_NUM_CAMERAS;
      c->num_encoders = JS50WSC_NUM_ENCODERS;
      c->num_lasers = JS50WSC_NUM_LASERS;
      break;
